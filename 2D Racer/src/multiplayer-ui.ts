...
export function createMultiplayerUI(root:HTMLElement,callbacks:{host:(endpoint:string,name:string,count:number)=>void;join:(endpoint:string,name:string,code:string)=>void;ready:(value:boolean)=>void;start:()=>void;leave:()=>void}) {
  ...
  controls.innerHTML=`<label>Your name<input class="racer-name" maxlength="16" placeholder="Racer" autocomplete="nickname"></label>
    <div class="host-row"><label>Players<select class="room-capacity"><option value="2">2 players</option><option value="3">3 players</option><option value="4" selected>4 players</option></select></label><button class="secondary host-button">HOST MULTIPLAYER</button></div>
  ...
  return {
    ...
    lobby(data:Lobby,sessionId:string){
      ...
      const start=q<HTMLButtonElement>('.race-start');start.hidden=sessionId!==data.hostId;start.disabled=data.members.length!==data.capacity || !data.members.every(m=>m.ready);
      q('.lobby-notice').textContent=start.hidden?'Waiting for the host to start once everyone is ready.':'Fill every seat and mark everyone Ready to start.';
    },
