...
export interface Car { id: number; x: number; z: number; y: number; heading: number; speed: number; steer: number; airborne: boolean; offroad: boolean; progress: number; color: string }
export interface RaceState { phase: Phase; cars: Car[]; remaining: number; countdown: number; elapsed: number; position: number; lapProgress: number }
